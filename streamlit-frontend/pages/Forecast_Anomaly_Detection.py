...
st.header("Predictions: Forecast future consumption")

days = st.slider("Days to forecast", 1, 30, 7)
homeid = st.text_input("Optional HomeID (leave blank for global forecast)", value="")
...
# Anomaly Detection 
homeid = st.text_input("HomeID to analyze (leave blank for all households):")
start = st.date_input("Start date")
end = st.date_input("End date")

if st.button("Detect Anomalies"):
    payload = {"HomeID": homeid} if homeid else {}
    # pass start/end as strings
    params = {}
    if start:
    ...
    resp = requests.get("http://localhost:7071/api/anomaly", params={**params, **({'HomeID':homeid} if homeid else {})})
    if resp.status_code == 200:
        ...
        if anom.empty:
            ...
            # Plot time series for the home (if homeid provided), and plot anomalies as red markers
            if homeid:
                # fetch the daily series for that home from your API / GetEnergyByHomeID or reuse the anomaly results
